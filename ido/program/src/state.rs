...
impl Pool {
    ...
    /// ```
    /// SIDE NOTE: allocation should be based on goal max collected, not distributed
    /// Thus, division by price is omitted intentionally
    ...
    pub fn stage_investment(
        ...
        let possible_amount = match (stage, tier) {
            (Stage::InitialStage, Some(tier)) => self.tier_allocation[tier],
            (Stage::TierAllocationStage, Some(tier)) => self.tier_allocation[tier],
            (Stage::FinalStage, Some(tier)) => self.tier_remaining[tier],
...
mod tests {
    ...
    fn pool_math_example() {
        ...
        assert_eq!(pool.tier_allocation[0], 3030303);
        assert_eq!(pool.tier_allocation[1], 5454545);
        assert_eq!(pool.tier_allocation[2], 9696969);
        assert_eq!(pool.tier_allocation[3], 18181818);
    }
    ...
    fn pool_math_equal() {
        ...
        let mut pool = pool_new(price, goal_max);
        pool.set_tier_allocations(tier_users, tier_balance).unwrap();
        assert_eq!(pool.tier_allocation[0], 25000000);
        assert_eq!(pool.tier_allocation[1], 25000000);
        assert_eq!(pool.tier_allocation[2], 25000000);
        assert_eq!(pool.tier_allocation[3], 25000000);
        assert_eq!(pool.tier_remaining[0], 250000000);
        assert_eq!(pool.tier_remaining[1], 250000000);
        assert_eq!(pool.tier_remaining[2], 250000000);
        assert_eq!(pool.tier_remaining[3], 250000000);
    }
    ...
    fn pool_one() {
        ...
        let mut pool = pool_new(price, goal_max);
        pool.set_tier_allocations(tier_users, tier_balance).unwrap();
        assert_eq!(pool.tier_allocation[0], 500000000);
        assert_eq!(pool.tier_allocation[1], 1000000000);
        assert_eq!(pool.tier_allocation[2], 1500000000);
        assert_eq!(pool.tier_allocation[3], 2000000000);
        assert_eq!(pool.tier_remaining[0], 0);
        assert_eq!(pool.tier_remaining[1], 1000000000);
    ...
    fn pool_skew() {
        ...
        let mut pool = pool_new(price, goal_max);
        pool.set_tier_allocations(tier_users, tier_balance).unwrap();
        assert_eq!(pool.tier_allocation[0], 16666666);
        assert_eq!(pool.tier_allocation[1], 16666666);
        assert_eq!(pool.tier_allocation[2], 16666666);
        assert_eq!(pool.tier_allocation[3], 3 * 16666666 + 2);
        assert_eq!(pool.tier_remaining[0], 16666666 * 10);
        assert_eq!(pool.tier_remaining[1], 16666666 * 10);
        assert_eq!(pool.tier_remaining[2], 16666666 * 10);
        assert_eq!(pool.tier_remaining[3], 500000000);

        assert_eq!(
            pool.tier_remaining.iter().sum::<u64>(),
            goal_max * Pool::PRECISION - 20
        );
    }
    ...
    fn pool_invest_math() {
        let goal_max = 1;
        let price = 1_000_000_000;
        let tier_balance = [3, 6, 9, 12];
        let tier_users = [100, 50, 25, 5];
        let mut pool = pool_new(price, goal_max);
        pool.set_tier_allocations(tier_users, tier_balance).unwrap();
        
        assert_eq!(
            pool.stage_investment(10000000, Stage::InitialStage, [3, 6, 9, 12], 7)
                .unwrap()
                .0,
            6779661
        );
        assert_eq!(
            pool.stage_investment(10000000, Stage::TierAllocationStage, [3, 6, 9, 12], 7)
                .unwrap()
                .0,
            6779661
        );
        assert_eq!(
            pool.stage_investment(1000000000, Stage::FinalStage, [3, 6, 9, 12], 7)
                .unwrap()
                .0,
            338983050
        );
