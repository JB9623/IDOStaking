...
use sol_starter_staking::{
    ...
    program::{create_account_with_seed_signed, ProgramPubkey, PubkeyPatterns},
...
impl Processor {
    ...
    pub fn participate<'a, 'b>(
        ...
        let (user_pool_key, user_pool_bump_seed) =
        ...
        let user_pool_stage_key =
            Pubkey::create_with_seed(&user_pool_key, seed.as_str(), &program_id.pubkey())?;
        ...
        if amount_collected == 0 {
            return Err(Error::TierAlreadyFull.into());
        }
