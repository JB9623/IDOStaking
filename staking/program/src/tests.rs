use crate::{
    id,
    instruction::{
        self, InitializePoolInput, LockInput, DoStakeInput, UnlockInput, UnstakeStartInput,
    },
    prelude::*,
...
async fn flow() {
    ...
    let instruction = instruction::do_stake(
        &pool.pubkey(),
        &pool_token_account_sos.pubkey(),
        &user_token_sos.pubkey(),
        &user_token_xsos.pubkey(),
    ...
    assert_eq!(account_state.amount, 990140);
